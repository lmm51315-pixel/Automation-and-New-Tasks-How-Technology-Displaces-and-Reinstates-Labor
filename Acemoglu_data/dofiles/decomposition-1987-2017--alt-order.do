...
for the years 1987-2017 using an alternative ordering of decomposition:
...
bys indcode: gen cum_substitution_i=100*(1-${sigma})*(1-base_labsh)*(logW_i-logW_i[1]-logR_i+logR_i[1]-(year-1987)*${growth_rate_1987_2017}) /*Formula for the substitution effect (see appendix)*/
...
twoway (connected  cum_prod_effect_wbill year, $style_prod_effect mlabel(label_prod)) ///
	   (connected  cum_comp year, $style_comp_effect mlabel(label_comp) mlabpos(2)) ///
	   (connected  cum_substitution_i year, $style_price_subs mlabel(label_subs)) ///
       (connected  cum_task_content_i year, $style_task_content mlabel(label_task)) ///
	   (connected  cum_delta_wbill_us year, $style_observed mlabel(label_wbill)), ///
       title("Wage bill, 1987-2017 (alternative ordering)", size(medium) position(11)) ytitle("")  xtitle("")  yscale(noline) ///
...
graph export "${project}\figs\decomposition_wbill_1987_2017_sigma_${sigma}_order.eps", as(eps) preview(on) replace
